...
public class GameSystem {
	...
	public BaseUnit promote(BaseUnit baseUnit, int choice) {
        ...
        int column = baseUnit.getColumn();
        boolean isWhite = baseUnit.isWhite();
        String name = baseUnit.getName();

        BaseUnit promotedUnit;
        
        switch (choice) {
            case 0: // Melee unit
                promotedUnit = new MeleeUnit(row, column, isWhite, name);
                break;
            case 1: // Range unit
                promotedUnit = new RangeUnit(row, column, isWhite, name);
                break;
            case 2: // Flying unit
                promotedUnit = new FlyingUnit(row, column, isWhite, name);
                break;
        ...
        // Remove old unit and add promoted unit
        ArrayList<BaseUnit> pieces = isWhite ? allWhitePieces : allRedPieces;
        pieces.remove(baseUnit);
        pieces.add(promotedUnit);
